...
function App() {
  ...
  },
  {
    ...
    img:image10,title:"BanatHero",languanges:["HTML","CSS","JS"],dateDeploy:"unknown"
  },
  {
      ...
      <div className=" box-border flex min-h-screen w-full flex-col overflow-x-hidden bg-background text-[#dadada87]  ">
        ...
        >
          ...
          <div className=" relative mt-60 flex h-[60%] flex-col gap-20  sm:ml-4  sm:mt-0 sm:h-0 sm:w-full  ">
            ...
            <br className=" sm:hidden" />Designer
          ...
          <div className="relative flex min-h-[10%] flex-col justify-start gap-20  sm:ml-4 sm:mt-0   sm:min-h-0 sm:gap-1">
            <div className=" flex flex-col  gap-1">
              ...
              <Reveal>
                ...
                {"Hey are looking for a Front-end or Designer".split("").map((child, idx) => (
              <span className={styles.hoverText} key={idx}>
                {child} 
              </span>
            ))}
                 <br />
                 {"to build your brand and grow you business?".split("").map((child, idx) => (
              <span className={styles.hoverText} key={idx}>
                {child} 
              </span>
            ))}
                  ...
                  {"let’s shake hands with me.".split("").map((child, idx) => (
              <span className={styles.hoverText} key={idx}>
                {child} 
              </span>
            ))}
              ...
              <a
                ...
                download={true}
                className=" animate__animated animate__fadeIn animate__faster animate__delay-1s mt-7   flex w-[40%]  items-center justify-center bg-primary/10  gap-2 rounded-md  py-2 font-inter  text-lg text-primary backdrop-blur-lg transition-all duration-200  hover:bg-primary/30  hover:text-accent-foreground active:translate-y-2 sm:mt-3  sm:w-[60%] sm:text-xs "
              >
                Download CV
